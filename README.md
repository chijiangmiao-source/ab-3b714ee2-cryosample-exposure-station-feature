...
- **误扫可撤销**：值班员无需改库或重建批次，可在同一批次页撤销**当前最后一条未撤销记录**，事件保留并附带可追溯的撤销时刻与原因。
  - 撤销**取出** → 批次回到柜内；撤销**归还** → 扣回该次暴露、恢复柜外（重开对应取出），并按恢复后的累计重新判定可用 / 报废。
  - 撤销须填写带 `Z` 的整秒时刻（须严格晚于批次最后操作）和非空原因。
  - 非最近事件、重复撤销、撤销时刻不晚于最后操作、并发导致的状态变化 → 明确 `409`，整事务回滚：不改累计、不改柜内外状态、不写流水审计。
  - 两个工位并发撤销同一事件，恰好一个成功。
...
### `POST /api/batches/:barcode/events/:id/revocation` — 撤销最近事件

误扫（误取出 / 误归还）后在同一批次页撤销，无需改库或重建批次。仅允许撤销**当前最后一条未撤销事件**：

```json
{ "at": "2026-09-13T08:05:00Z", "reason": "误扫归还，样本实际仍在柜外" }
```

- `at`：撤销时刻（带 `Z` 的 RFC3339 整秒，须**严格晚于**批次最后操作时刻）；
- `reason`：非空撤销原因（≤500 字符），随事件长期保留用于追溯。
- `200` → 撤销并反向更新后的批次 JSON：
  - 撤销取出 → `state: "in"`，批次回到柜内；
  - 撤销归还 → `state: "out"`、重开对应取出、`accumulatedSeconds` 扣回该次暴露，并按恢复后累计重新判定 `usable/scrapped`（误归还导致的报废可由此恢复）。
- `400 invalid_time` / `400 reason_required` / `400 invalid_event_id`：参数非法；
- `404 not_found`：条码或事件 id 不存在；
- `409 not_last_event`：目标事件不是最后一条未撤销记录（含非最近事件）；
- `409 already_revoked`：该事件已被撤销（重复撤销）；
- `409 time_not_monotonic`：撤销时刻未严格晚于批次最后操作；
- `409 invalid_transition`：并发下批次状态已被其他工位改变。

所有冲突均为**整事务回滚**：不写撤销信息、不改累计、不改柜内外状态；页面收到冲突后会重新载入服务端权威状态。撤销成功后可继续按恢复后的正确状态正常操作。

### `GET /api/batches/:barcode/events` — 事件流水

`200` → `{ "events": [ { "id", "type", "at", "deltaSeconds", "revokedAt", "revokeReason" }, ... ] }`（按写入顺序）。

- 已撤销事件**仍保留**在流水中，并带 `revokedAt`（撤销时刻）与 `revokeReason`（撤销原因）；未撤销事件不包含这两个字段。
- 批次查询的 `lastEvent` 回退为最后一条**未撤销**事件（全部撤销后为 `null`）。
- `revokedAt` / `revokeReason` 为新增可选字段：忽略未知字段的旧客户端仍可正常创建、查询、提交原有事件。
...
`verify` 针对运行中的整套服务执行验收并打印 `PASS/FAIL`，覆盖：创建与初始状态、重复条码 409、时间不单调 409、无取出归还 409、连续取出 409、**临界归还（累计==上限）仍可用**、**超限归还立即报废**、报废后取出 409、**重复归还不二次计时**、**8 路并发取出/归还仅一个成功且暴露只计一次**、拒绝的事件不落库、最终状态可重读（刷新一致），以及撤销能力：**误归还导致报废后撤销恢复柜外与原累计并可再次归还**、未超限误归还撤销后正确归还保持可用、**误取出撤销回到柜内**、非最近事件 409、重复撤销 409、撤销时刻不晚于最后操作 409、撤销参数校验 400、**8 路并发撤销仅一个成功且审计只写一次**、未使用撤销时原临界流程结果一致、前端页面可访问。全部通过以退出码 `0` 结束，否则非零。
